...
def nested_optimization(config, x_0, bounds, results=False, savePlots=False, saveInnerLog=None):   
    ...
    supports all 3 configurations: helicopter, quadcopter and octocopter.
    """
    # Determine aircraft type
    print('Determining aircraft type...')
    print(config)
...
if __name__ == "__main__":
    ...
    # quad.display()
    # #u, v, w, mag = computeVelocityField(data, points=None, plane='XY', discretization=10, plotting=True)
    #nested_optimization(config='base_helicopter.json', x_0=[400, 8000], bounds=[(300, 500), (7000, 12000)], results=False, savePlots=True, saveInnerLog='./DesignSpace/helicopter_simple')
    # helicopter = defineDrone('base_helicopter.json')
    # Gammas, FM, created_moment, Torque, Thrust, power_required, induced_power, profile_power, v_axial, STALL_TUPLE = solve(helicopter, case='base_helicopter.json', save=True, savePath='DesignSpace/helicopter_simple')
    # drone = defineDrone('base.json')
    # Gammas, FM, created_moment, Torque, Thrust, power_required, induced_power, profile_power, v_axial, STALL_TUPLE = solve(drone, case='base.json', save=True, savePath='DesignSpace/main_simple')