...
def plot_axial_force():
    ...
    data_LLM = np.load(f'./DesignSpace/test_helicopter/_res.npz')
    ...
    # cfd data 
    axial_cumulative = np.genfromtxt(f'./cfdResults/helicopter2/axial', skip_header=4, skip_footer=1)
    ...
    plt.xlabel('r/R')
    plt.ylabel(r'$F_{axial} [N/1m^2]$')

    plt.legend()
    plt.grid(True, linestyle='--', linewidth=0.5)
    plt.tight_layout()
    #plt.show()
    plt.savefig('./DesignSpace/test_helicopter/axial_force_comparison.png', dpi=300)  
...
def plot_tan_force():
    ...
    # cfd data 
    axial_cumulative = np.genfromtxt(f'./cfdResults/helicopter2/tangential', skip_header=4, skip_footer=1)
    axial_cfd = cumulative_to_differentiated(axial_cumulative[:,0], axial_cumulative[:,1])
    ...
    plt.plot(r[:n], f_axial_LLM[:n], label='LLM', marker='x', linestyle='--', color=design['colors'][1])
    plt.xlabel('r/R')
    plt.ylabel(r'$F_{tan} [N/1m^2]$')

    plt.legend()
    plt.grid(True, linestyle='--', linewidth=0.5)
    plt.tight_layout()
    #plt.show()
    plt.savefig('./DesignSpace/test_helicopter/tan_force_comparison.png', dpi=300)  
