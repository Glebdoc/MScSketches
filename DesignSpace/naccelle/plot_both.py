...
drone = np.load('./drone/_res.npz')
drone_base = np.load('./drone/_res_base.npz')
drone = drone['data']
drone_base = drone_base['data']
heli = np.load('./helicopter/_res.npz')
heli = heli['data']

cl_cd_heli = heli['Cl']/heli['Cd']
cl_cd_drone = drone['Cl']/drone['Cd']
cl_cd_base = drone_base['Cl']/drone_base['Cd']

axs[0].plot(drone['r'][:n], cl_cd_base[:n], label='spinning drone', marker='o', color=design['colors'][0])
axs[0].plot(heli['r'][:n], cl_cd_heli[:n], label='helicopter', marker='v', color=design['colors'][1])
axs[0].set_ylabel(r'$C_l / C_d$')
axs[0].set_xlabel(r'$r/R$')
axs[0].grid()
axs[0].legend()


axs[1].plot(drone['r'][:n], cl_cd_drone[:n], label='spinning drone', marker='o', color=design['colors'][0])
axs[1].plot(heli['r'][:n], cl_cd_heli[:n], label='helicopter', marker='v', color=design['colors'][1])
...
print(np.max(((cl_cd_base[:n] - cl_cd_heli[:n])/cl_cd_base[:n])*100))
print(np.max(((cl_cd_drone[:n] - cl_cd_heli[:n])/cl_cd_drone[:n])*100))
